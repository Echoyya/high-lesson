...
class Promise {
  constructor(executor) {
    ...
    let resolve = (value) => {
      // 一个promise 直接resolve 一个promise的情况,递归判断 并处理then方法
    ...
    return new Promise((resolve,reject)=>{
      ...
      for (let i = 0; i < promises.length; i++) { // 并发。多个请求一起执行
        ...
        if(p && typeof p.then === 'function'){
          ...
          },reject) // 如果其中一个promise失败了，直接执行失败回调即可，由于reject本身就是函数，故可以简写
        }else{
          processSuccess(i,p)
        }
      }
  ...
  static race(promises){
    // race 赛跑，采用最快的那一个，race方法如果其中一个完成了，其他方法还是会执行完，只是并没有采用他的结果
    return new Promise((resolve,reject)=>{
